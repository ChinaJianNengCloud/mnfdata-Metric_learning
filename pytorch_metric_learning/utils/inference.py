...
class InferenceModel:
    def __init__(
        ...
        self.classifier = torch.nn.Identity() if classifier is None else classifier
        self.match_finder = (
    ...
    def get_embeddings_from_tensor_or_dataset(self, inputs, batch_size):
        ...
        if isinstance(inputs, (torch.Tensor, list)):
            ...
            for inp, _ in dataloader:
                embedding,yinda_output=self.get_embeddings(inp)
                embeddings.append(embedding)
                yinda_outputs.append(yinda_output)
        else:
            raise TypeError(f"Indexing {type(inputs)} is not supported.")
        return torch.cat(embeddings),torch.cat(yinda_outputs)
    ...
    def call_knn(self, func, inputs, batch_size):
        embeddings,classifier = self.get_embeddings_from_tensor_or_dataset(inputs, batch_size)
        func(embeddings)
    ...
    def get_embeddings(self, x):
        ...
        with torch.no_grad():
            ...
            yinda_output=self.classifier(x_emb)
        if self.normalize_embeddings:
            x_emb = torch.nn.functional.normalize(x_emb, p=2, dim=1)
        return x_emb,yinda_output
