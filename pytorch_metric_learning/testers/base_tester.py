...
class BaseTester:
    ...
    def compute_all_embeddings(self, dataloader, trunk_model, embedder_model,classifier_model):
        s, e = 0, 0
        with torch.no_grad():
            for i, data in enumerate(tqdm.tqdm(dataloader)):
                img, label = self.data_and_label_getter(data)
                label = c_f.process_label(label, "all", self.label_mapper)
                q,yinda = self.get_embeddings_for_eval(trunk_model, embedder_model,classifier_model, img)
    ...
    def get_all_embeddings(
        ...
        if classifier_model    is None:
                classifier_model    = torch.nn.Identity()
        if eval:
            trunk_model.eval()
            embedder_model.eval()
        ...
        embeddings, classifier,labels = self.compute_all_embeddings(
            dataloader, trunk_model, embedder_model,classifier_model
        )
    ...
    def get_embeddings_for_eval(self, trunk_model, embedder_model, classifier_model,input_imgs):
        ...
        if self.use_trunk_output:
            return trunk_output
        embedder_output=embedder_model(trunk_output)
        classifer_output=classifier_model(embedder_output)
        return embedder_output,classifer_output
