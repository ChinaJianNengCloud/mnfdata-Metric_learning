...
if __name__ == '__main__':
    ...
    softmax_scales= [0.5,1,2,4]
    esplionss = [0.05, 0.1, 0.2, 0.25, 0.3, 0.4]
    for i in range(30):
        embedding_output = random.choice(embedding_outputs)
        softmax_scale = random.choice(softmax_scales)
        esplions = random.choice(esplionss)
        exp_name = f"shiyan11-NCAloss-embeding_output{embedding_output}-softmax_scale{softmax_scale}-esplions{esplions}--{i}"
        # exp_name = "test"
        log_dir = fr"/root/autodl-tmp/result/Lossresult/youhua/NCAloss/{exp_name}/log"
        tensorboard_dir = fr"/root/autodl-tmp/result/Lossresult/youhua/NCAloss/{exp_name}/tensorboard"
        model_save_dir = fr"/root/autodl-tmp/result/Lossresult/youhua/NCAloss/{exp_name}/model"
        ...
        loss = losses.NCALoss(softmax_scale=softmax_scale)
        classification_loss = torch.nn.CrossEntropyLoss()
        miner = miners.MultiSimilarityMiner(epsilon=esplions)
