...
if __name__ == '__main__':
    ...
    esplions = [0.05, 0.1, 0.2, 0.25, 0.3, 0.4]
    num_bins=[5,10,15,20,30]
    for i in range(30):
        embedding_output = random.choice(embedding_outputs)
        esplion = random.choice(esplions)
        num_bin = random.choice(num_bins)
        exp_name = fr"shiyan11-FastAploss-embeding_putput{embedding_output}-esplion{esplion}-num_bins{num_bin}-{i}"
        log_dir = fr"/root/autodl-tmp/result/Lossresult/youhua/FastAPloss/{exp_name}/log"
        tensorboard_dir = fr"/root/autodl-tmp/result/Lossresult/youhua/FastAPloss/{exp_name}/tensorboard"
        model_save_dir = fr"/root/autodl-tmp/result/Lossresult/youhua/FastAPloss/{exp_name}/model"
        freeze_support()
        ...
        loss = losses.FastAPLoss(num_bins=num_bin)
        classification_loss = torch.nn.CrossEntropyLoss()
        miner = miners.MultiSimilarityMiner(epsilon=esplion)
