...
class MultiBranchConv1DModel(nn.Module):
    def __init__(self):
        ...
        # 并行卷积模块
        self.branch1 = nn.Conv1d(in_channels=1, out_channels=16, kernel_size=3, padding=1)
        self.branch2 = nn.Conv1d(in_channels=1, out_channels=16, kernel_size=5, padding=2)
        self.branch3 = nn.Conv1d(in_channels=1, out_channels=16, kernel_size=7, padding=3)

        # 合并后的卷积和全连接层
        self.conv_merge = nn.Conv1d(in_channels=48, out_channels=64, kernel_size=3, padding=1)
        self.fc = nn.Linear(64 * 32, 32)
...
if __name__ == '__main__':
    ...
    exp_name="moreoneconv"
    log_dir=fr"/root/autodl-tmp/result/{exp_name}/log"
    tensorboard_dir=fr"/root/autodl-tmp/result/{exp_name}/tensorboard"
    model_save_dir=fr"/root/autodl-tmp/result/{exp_name}/model"
    freeze_support()
    ...
    summary(trunk, (trunk_output_size,))
    embedder = torch.nn.DataParallel(MLP([trunk_output_size, 32]).to(device))
    summary(embedder, (trunk_output_size,))
    classifier = torch.nn.DataParallel(MLP([32,2])).to(device)
    summary(classifier, (trunk_output_size,))
    # Set optimizers
    ...
    train_dataset = mnfDataset(file_path='/root/autodl-tmp/对数归一化后的数据（十七支队）.xlsx', sheet_name='训练集',transform=None,device=device)
    val_dataset =mnfDataset(file_path='/root/autodl-tmp/对数归一化后的数据（十七支队）.xlsx', sheet_name='西溪验证集',transform=None,device=device)
    hold_out_dataset = mnfDataset(file_path='/root/autodl-tmp/对数归一化后的数据（十七支队）.xlsx', sheet_name='留出集', transform=None,
                             device=device)
    zheyi_dataset = mnfDataset(file_path='/root/autodl-tmp/对数归一化后的数据（十七支队）.xlsx', sheet_name='浙一验证集', transform=None,
                             device=device)
    ...
    end_of_epoch_hook = hooks.end_of_epoch_hook(
        tester, dataset_dict, model_folder, test_interval=1, patience=400
    )
